...
```{r, eval=FALSE, echo=FALSE}
# failed sp attempt
ways_touching_sp = as(object = ways_touching, Class = "Spatial")
route_single_sp = as(route_single, "Spatial")
sel_overlap = over(ways_touching_sp, route_single_sp, minDimension = 1, returnList = F)
```
...
```{r}
n_cores <- 8 # set max number of cores to 4
if(parallel:::detectCores() < 4)
  n_cores <- parallel:::detectCores()
cl <- parallel::makeCluster(n_cores)
doParallel::registerDoParallel(cl)
l$quietness_ways = NA # the variable to calculate

for(i in 1:nrow(l))
  l$quietness_ways[i] = mean(ways[lfq$rf[i,],]$quietness, na.rm = T) ; print(i)


quietness_by_length = l$busyness_fast / l$dist /1000
