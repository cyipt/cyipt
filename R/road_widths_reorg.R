...
#for(a in 588:nrow(osm)){
  ...
  roadside <- st_cast(roadside, "POLYGON", group_or_split=TRUE)
  ...
  road <- st_cast(road, "POLYGON", group_or_split=TRUE)
  rm(os_sub)
  ...
  #Get Road Width
  if(nrow(road) > 0){
    #print("Doing Road Width")
    #Get Road Dimentions
    road$area <- as.numeric(st_area(road))
    #print(class(road$geoms))
    ...
    print("Unable to get road width data")

    plot(AOI[,1],  col = rgb(0, 0, 0,0), main = paste0("Unable to get road width data ",a))
    plot(road[,1], add = T, col = "Green")
    plot(roadside, add = T, col = "Red")
  ...
  # Get Path Width
  ###################################
  if(nrow(roadside) > 0 & nrow(road) > 0){
    if(!is.na(road_main$OBJECTID[1])){
      #print("Getting Roadside width")
      #Select raodside touching the road section
      ...
      #print("Convert Multipolygons into single polygons")
      ...
      #print("Remove Roadside that does not touch the road")

      #Remove Roadside that does not touch the road
      roadside_notouch <- roadside[st_touches(road_main, roadside)[[1]],]

      #Check that there are roadsides left
      if(nrow(roadside_notouch) > 0){
        roadside <- roadside_notouch
        ...
        #Use accuate road width if possible lese use approximate
        #print("Use accuate raod width if possible lese use approximate")
        for(e in 1:nrow(roadside)){
          if((roadside$perimeter[e] ^ 2) > (16 * roadside$area[e])){
        ...
        print("No roadside touching the road")

        plot(AOI[,1],  col = rgb(0, 0, 0,0), main = paste0("No roadside touching the road ",a))
        plot(road[,1], add = T, col = "Green")
        plot(roadside, add = T, col = "Red")
        plot(osm_sub[,1], add = T, col = "Blue", lwd = 3)
      ...
      print("No main road polygon identified")

      #For cases where there is no road but their is roadside e.g. off road cycle path
      print("Check for roadside only situation 2")
      roadside_only <- roadside[st_intersects(osm_sub,roadside)[[1]],]
      if(nrow(roadside_only) > 0){
        ...
        plot(AOI[,1],  col = rgb(0, 0, 0,0), main = paste0("No main road polygon identified ",a))
        plot(road[,1], add = T, col = "Green")
        plot(roadside, add = T, col = "Red")
        plot(osm_sub[,1], add = T, col = "Blue", lwd = 3)
  ...
  }else if(nrow(roadside) > 0){
    #For cases where there is no road but their is roadside e.g. off road cycle path
    print("Check for roadside only situation 1")
    roadside_only <- roadside[st_intersects(osm_sub,roadside)[[1]],]
    if(nrow(roadside_only) > 0){
      roadside_only$area <- as.numeric(st_area(roadside_only))
      roadside_only$perimeter <- sum(as.numeric(st_length(roadside_only, dist_fun = geosphere::distGeo)))
    ...
    print("Unable to get roadside width data")

    plot(AOI[,1],  col = rgb(0, 0, 0,0), main = paste0("Unable to get roadside width data ",a))
    plot(road[,1], add = T, col = "Green")
    plot(roadside, add = T, col = "Red")
    plot(osm_sub[,1], add = T, col = "Blue", lwd = 3)
