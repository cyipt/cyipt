...
for(b in seq_len(1:length(regions))){
  ...
  if(all(filecheck)){
    ...
    if(class(schemes)[1] != "numeric"){
      ...
      if(all(c("INSERT COLUMNS HERE") %in% names(schemes)) & skip){
        ...
        col.to.keep <- names(schemes)[!names(schemes) %in% c("INSERT COLUMNS HERE")]
        ...
        for(j in scheme_nos){
          ...
          route.summary <- lapply(1:nrow(pct.scheme), get.exposure)
          ...
          #Health Benefits
          healthbens <- lapply(1:nrow(pct.scheme), cyipt.heath)
          healthbens <- bind_rows(healthbens)
          pct.scheme <- left_join(pct.scheme, healthbens, by = c("ID" = "id"))
          rm(healthbens)
          ...
          benefits_list <- c("absenteeism_benefit", "health_benefit",
                             "accidents_benefit","noise_benefit","co2saved","ghg_benefit",
