...
for(b in 1:length(regions)){
  if(file.exists(paste0("../cyipt-bigdata/osm-prep/",regions[b],"/osm-lines.Rds"))){
    ...
    if(file.exists(paste0("../cyipt-bigdata/osm-prep/",regions[b],"/pct-up.Rds")) & skip){
      ...
      model <- readRDS("../cyipt/input-data/m2.Rds")
      ...
      for(j in scheme_nos){
       ...
       # }
        ...
        pct.scheme$yearcomplete <- 5 #placeholder duration

        # New Exposure Method
        pct.scheme.mat <- as.data.frame(pct.scheme[,c("length","exposeOffRoad","exposeOnRoad","yearcomplete")])
        pct.scheme.mat$geometry <- NULL
        names(pct.scheme.mat) <- c("dist", "length_on_road", "length_off_road", "years_complete")
