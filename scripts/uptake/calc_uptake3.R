...
for(b in 1:length(regions)){
  if(file.exists(paste0("../cyipt-bigdata/osm-prep/",regions[b],"/osm-lines.Rds"))){
    ...
    osm <- readRDS("../cyipt-bigdata/osm-prep/Bristol/osm-lines-pct2-test.Rds")
    ...
    if(all(c("FILL ME IN") %in% names(osm)) & skip){
      ...
      for(j in scheme_nos){
        ...
        pct.scheme$model.before <-  round(exp(-2.2679333871 -0.0001207765 * pct.scheme$length + 0.0242742120 * sqrt(pct.scheme$length) -0.0075675068 * sqrt(pct.scheme$busyBefore) -1.9347971306 * pct.scheme$av_incline + -6.8377711996 * sqrt(pct.scheme$av_incline)),1) * pct.scheme$total
        pct.scheme$model.after <-  round(exp(-2.2679333871 -0.0001207765 * pct.scheme$length + 0.0242742120 * sqrt(pct.scheme$length) -0.0075675068 * sqrt(pct.scheme$busyAfter) -1.9347971306 * pct.scheme$av_incline + -6.8377711996 * sqrt(pct.scheme$av_incline)),1) * pct.scheme$total

        #Uptake for scheme is
        uptake <- data.frame(scheme = j, census = sum(pct.scheme$pct.census), model.now = sum(pct.scheme$model.before), model.future = sum(pct.scheme$model.after))
